...
self.addEventListener('sync', (event) => {
    if (event.tag === 'practice-sync') {
        event.waitUntil(syncPracticeData());
    }
});

async function syncPracticeData() {
    // Placeholder for syncing practice data when online
...
self.addEventListener('push', (event) => {
    const options = {
        body: event.data ? event.data.text() : 'Time for math practice!',
    ...
    event.waitUntil(
        self.registration.showNotification('Math Practice', options)
    );
});
